...
export const metadata: Metadata = {
  title: 'Home',
  description: "Welcome to Fiston's portfolio - Explore my projects and blog posts",
}
    ...
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <main className="flex-grow container mx-auto px-4 py-16">
        ...
        <p className="mb-8 max-w-2xl text-gray-700 dark:text-gray-300">
          Software developer with a knack for solving problems. I write clean code and
          bad jokes. Always learning, rarely napping. Let&apos;s build something cool together.
        </p>
          ...
          {projects.length > 0 ? (
            projects.slice(0, 2).map((project: Project) => (
              <article key={project.sys.id}>
                <h3 className="text-xl font-semibold">
                  <Link href={`/projects/${project.fields.slug}`} className="hover:underline">
                    {project.fields.title}
                  </Link>
                </h3>
                <p className="text-gray-600 dark:text-gray-400">{project.fields.description}</p>
              </article>
            ))
          ) : (
            <p>No projects found. Check Contentful for the &apos;project&apos; content type.</p>
          )}
        </section>
